...
# ANTES DE RODAR, RODAR O FREE FILE SYNC

local_arquivo <- c(here::here("data-raw/refugiados/Entrevista.xlsx"))
...
profissao_siria <- stringr::str_to_lower(df$profissao_na_siria)
profissao_egito <- stringr::str_to_lower(df$profissao_no_egito)
...
make_consulta_vitem <- function(numero, nome, sexo, pais, nascimento, nome_pai, nome_mae, passaporte,
  ...
  if (idade < 18) {
    ...
    if (sexo == "Masculino") {
      ...
      observacoes <- glue::glue("filho menor de {nome_pai}")
    
    }
    
    else {
      
      observacoes <- glue::glue("filha menor de {nome_pai}")
...
texto <- map(1:nrow(df), ~ make_consulta_vitem(
  numero = .x,
  nome = nome[.x],
  pais = pais[.x],
  sexo = sexo[.x],
...
texto %>% 
  ...
  stringr::str_replace_all("Siria, em", "Síria, em") |> 
  stringr::str_replace_all("Não;", "não;") |> 
  stringr::str_replace_all("NA", "não") |> 
  stringr::str_replace_all("Masculino;", "masculino;") |> 
  stringr::str_replace_all("Feminino;", "feminino;") |> 
  stringr::str_replace_all("Não tem\\.", "não tem endereço") |>
  stringr::str_replace_all("Nao", "não") |> 
  stringr::str_replace_all("endereço no Brasil: Não tem", "endereço no Brasil: não tem endereço") |>
  stringr::str_replace_all("estudante\\.", "estudante") |> 
  stringr::str_replace_all(" ;", ";") |> 
  stringr::str_replace_all("profissão: Não teve*", "Não declarada") |> 
  stringr::str_replace_all("endereço no Brasil: Sem.", "endereço no Brasil: não declarado") |> 
  stringr::str_replace_all("Não determinada\\.", "não determinada") |>
  stringr::str_replace_all("Embassy-", "Embaixada em ") |> 
  
  write_lines("vitem_iii_base.txt", sep = "\n\n", append = T)

write_lines("Antonio de Aguiar Patriota, Embaixador", "vitem_iii_base.txt", append = T)

# write_lines("Rubem Mendes de Oliveira, Encarregado de Negócios, a.i.,", "vitem_iii_base.txt", append = T)
